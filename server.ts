...
const serverAuth = getAuth(serverFbApp);

// Automatically authenticate server session to prevent Firestore unauthenticated write rejections
signInAnonymously(serverAuth)
  .then(() => {
    console.log("[SERVER FIREBASE] Anonymous server authentication established successfully.");
  })
  .catch((authErr) => {
    console.warn("[SERVER FIREBASE] Anonymous auth warning:", authErr.message || authErr);
  });
...
async function startServer() {
  ...
  // =========================================================================
  // 1:1 Snap (인연 스냅) Server-side Synchronization & Disk Persistence APIs
  // =========================================================================
  const DATA_DIR = path.resolve(process.cwd(), ".data");
  const SNAP_FILE_PATH = path.join(DATA_DIR, "pair_snaps.json");
  ...
  if (!fs.existsSync(DATA_DIR)) {
    try {
      ...
      console.warn("[SERVER SNAP PERSISTENCE] Directory creation warning:", dirErr);
    }
  }

  const pairSnapsCache = new Map<string, any>();

  // Hydrate snaps cache from disk on server startup
  try {
    if (fs.existsSync(SNAP_FILE_PATH)) {
      const rawData = fs.readFileSync(SNAP_FILE_PATH, "utf-8");
      const parsedSnaps = JSON.parse(rawData);
      for (const [key, value] of Object.entries(parsedSnaps)) {
        pairSnapsCache.set(key.toUpperCase().trim(), value);
      }
      console.log(`[SERVER SNAP PERSISTENCE] Restored ${pairSnapsCache.size} snaps from disk.`);
    }
  } catch (loadErr) {
    console.warn("[SERVER SNAP PERSISTENCE] Hydration warning:", loadErr);
  }

  // Helper to persist in-memory cache to disk reliably
  function persistSnapsToDisk() {
    try {
      const dump: Record<string, any> = {};
      for (const [k, v] of pairSnapsCache.entries()) {
        dump[k] = v;
      }
      fs.writeFile(SNAP_FILE_PATH, JSON.stringify(dump, null, 2), "utf-8", (wErr) => {
        if (wErr) console.warn("[SERVER SNAP PERSISTENCE] Disk write warning:", wErr);
      });
    } catch (e) {
      console.warn("[SERVER SNAP PERSISTENCE] Write failure:", e);
    }
  }
...
8. **[전체 모임 화합을 높이는 팁(synergy_tips) 줄바꿈 가이드]:**
   - '화합을 높이는 팁(synergy_tips)' 작성 시 DO 항목과 DON'T 항목 사이에는 반드시 명확한 줄바꿈('\\n\\n')을 넣어 문단을 분리하십시오.
